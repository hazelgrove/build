...
`;
  ...
  window.HazelJSCoq = {
    ...
    searchRequests: /* @__PURE__ */ new Map(),
    completedChecks: [],
    ...
    installHiddenManagerGuards(manager) {
      ...
      manager.coqCoqExn = ({ pp, msg, sids } = {}) => {
        const activeSentence = manager.doc && manager.doc.sentences ? manager.doc.sentences.slice().reverse().find((stm) => stm && (stm.phase === "pending" || stm.phase === "adding" || stm.phase === "added" || stm.phase === "processing")) : null;
        const sid = sids && sids.length > 0 ? sids[0] : activeSentence && activeSentence.coq_sid ? activeSentence.coq_sid : 0;
        markError(sid, void 0, pp || msg);
      };
    ...
    async makeManager({ code: code3 = "", show = true, fresh = false } = {}) {
      ...
      const needsReals = !fresh || code3.includes("Require Import Reals");
    ...
    cancelManagerWork(manager) {
      ...
      const activeSentence = manager.doc.sentences.slice().reverse().find(
      ...
      try {
        ...
        else if (manager.coq && typeof manager.coq.cancel === "function" && sid) {
          manager.coq.cancel(sid);
        } else return false;
        return true;
      } catch (error) {
    ...
    managerNeedsRecycle(manager = this.manager) {
      const health = this.managerHealth(manager);
      return health.highestStateId >= this.maxPersistentStateId || health.userChecks >= this.maxPersistentUserChecks;
    },
    ...
    waitForActiveChecks({ request = null, timeoutMs = 5e3 } = {}) {
    ...
    async start({ code: code3 = "", show = true } = {}) {
      ...
      if (!this.managerPromise) {
        this.managerPromise = this.makeManager({ code: code3, show, fresh: false }).then((manager) => {
    ...
    async replaceManagerDocument(manager, code3) {
      const prefixSentences = manager.__hazelWarmupPrefixSentences || manager.doc.sentences.slice(0, 1);
      const firstState = manager.doc.sentences.slice(prefixSentences.length).find((stm) => stm && stm.coq_sid);
      if (firstState) {
        ...
        await new Promise((resolve, reject) => {
          ...
          const poll = () => {
            ...
            else if (performance.now() - startedAt > 3e3) {
      ...
      const suffix = manager.__hazelWarmupPrefixCode ? code3.replace(/^From (?:Coq|Stdlib) Require Import [^\n]*\.\s*$/gm, "") : code3;
    ...
    async check(code3, { show = true, advanceLimit = 200, kind = "validation", request = null } = {}) {
      ...
      const result = {
        ...
        steps,
        errors: activeCheck.aborted ? [activeCheck.abortReason || "JSCoq check was cancelled."] : manager.__hazelAborted ? ["JSCoq check was aborted."] : manager.error,
    ...
    warmupSearch(opts = {}) {
      ...
      if (!this.warmupPromise) {
        ...
        const warmup = this.start({ code: "", show: false }).then(async (manager) => {
          ...
          console.log(
            "[Hazel JSCoq]",
            result.ok ? "Persistent Rocq worker and libraries ready." : "Persistent Rocq worker failed.",
    ...
    checkAndReport(code3, callback, opts = {}) {
      const request = opts.request || null;
      const maxDiagnosticDepth = 24;
      const maxDiagnosticNodes = 800;
      const maxDiagnosticChars = 6e3;
      const truncateDiagnostic = (text) => text.length <= maxDiagnosticChars ? text : `${text.slice(0, maxDiagnosticChars)}\u2026 [Rocq diagnostic truncated]`;
      const errorText = (error, seen = /* @__PURE__ */ new Set(), depth = 0, budget = { nodes: 0 }) => {
        ...
        if (Array.isArray(error)) {
          return truncateDiagnostic(
            error.map((item) => errorText(item, seen, depth + 1, budget)).filter(Boolean).join(" ")
          );
        }
      ...
      return this.warmupSearch().then((warmup) => {
        ...
        return Promise.race([this.check(code3, { show: false, ...opts }), timeout]).then((result) => {
          ...
          const fatalWorkerError = result && result.errors && result.errors.some(
            (error) => errorText(error).includes("Stack overflow")
          );
...
${formatErrors(result.errors)}`;
    ...
    searchAndReport(code3, callback, opts = {}) {
      const kind = code3.includes("hazel_profile_search") ? "profile search" : "equivalence fallback";
      console.log("[Hazel Rocq debug] generated program", {
        kind,
        codeBytes: code3.length
      });
      const requestId = opts.requestId;
      const request = requestId == null ? null : {
        id: requestId,
        callback,
        activeCheckId: null,
        cancelled: false,
      ...
      if (this.debugSkipRocqSearch) {
        ...
        const message = `JSCoq execution disabled for debugging; captured ${kind} program in window.HazelJSCoq.debugPrograms.`;
      ...
      const runWhenIdle = this.warmupSearch().then(() => this.waitForActiveChecks({ request })).then((waitResult) => {
